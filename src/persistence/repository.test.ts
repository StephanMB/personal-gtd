...
test('legacy data is migrated in memory and saving never touches the legacy key', () => {
  ...
  assert.equal(repo.save({ items: loaded.items, projects: [], settings: {} }).ok, true);
...
test('current data wins over legacy data', () => {
  const current = JSON.stringify({ schemaVersion: SCHEMA_VERSION, items: [], projects: [], settings: {} });
...
test('corrupt, newer and unavailable are reported, never thrown', () => {
  ...
  assert.equal(throwing.save({ items: [], projects: [], settings: {} }).ok, false);
...
test('save failures are returned, not thrown', () => {
  ...
  const r = repo.save({ items: [], projects: [], settings: {} });
